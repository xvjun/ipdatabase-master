...
//        BufferedReader ipRelationReader = FileUtil.readFile(file);
        //修改后
        InputStream file = IpHelper.class.getClassLoader().getResourceAsStream(ipFile);
        BufferedReader ipRelationReader = new BufferedReader(new InputStreamReader(file));
...
//        String file =  IpHelper.class.getClassLoader().getResource(regionFile).getFile();
        ...
        InputStream file = IpHelper.class.getClassLoader().getResourceAsStream(regionFile);
...
//        Workbook workbook = PoiUtil.getWorkbook(file);
        ...
        for (int i = 1; i < rowLen; i++) {
            ...
            String provinceCity = row.getCell(1).getStringCellValue();
            Double a = row.getCell(2).getNumericCellValue();
            Integer ipCode = a.intValue();
            map.put(ipCode, province+" "+provinceCity);
        }
